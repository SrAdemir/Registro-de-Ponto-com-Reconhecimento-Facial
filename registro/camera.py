...
class VideoCamera(object):
    def __init__(self):
        ...
        self.video = cv2.VideoCapture(0)  # Abertura da câmera no meu caso é pc

        if not self.video.isOpened(): # Verifica se camera está aberta
           print("Erro: erro ao acessar a câmera.")
    ...
    def detect_face(self):
        ...
        cv2.ellipse(frame, (centro_x, centro_y), (a, b), 0, 0, 360, (0, 0, 255), 10)  # Desenha elipse na área de interesse

        for (x, y, w, h) in faces:
            cv2.ellipse(frame, (centro_x, centro_y), (a, b), 0, 0, 360, (0, 255, 0), 10)  # Desenha elipse na área de interesse

        ret, jpeg = cv2.imencode('.jpg', frame)  # Codifica o frame como JPEG
        return jpeg.tobytes()  # Retorna os bytes da imagem JPEG
    ...
    def sample_faces(self, frame):
        ...
        for (x, y, w, h) in faces:
            cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 4)  # Desenha um retângulo ao redor da face detectada
